...
# def sys_normalized_adjacency(adj):
#     # adj: torch tensor (dense) or scipy sparse convertible
#     adj = sp.coo_matrix(adj.to_dense().cpu().numpy())
#     adj = adj + sp.eye(adj.shape[0], format="coo")

#     row_sum = np.asarray(adj.sum(1)).flatten()  # shape (N,)
...
def get_dataset(dataname, top_k,adj, features, labels, idx_train, idx_val, idx_test, sens, sens_idx):
    ...
    # Coalesce sparse tensor to ensure it's in canonical form
    adj_norm_sp = adj_norm_sp.coalesce()
...
class FairVGNN():
    ...
    def run(self, data, args):
        ...
        # --- DEBUG: sanity checks for CUDA asserts ---
        ei = data.edge_index
        n = data.x.size(0)

        print("edge_index device:", ei.device, "dtype:", ei.dtype, "shape:", tuple(ei.shape))
        print("num_nodes:", n)
        print("edge_index min/max:", int(ei.min().item()), int(ei.max().item()))
        ...
        assert torch.isfinite(data.x).all()
        # Coalesce sparse tensor before accessing values
        adj_coalesced = data.adj_norm_sp.coalesce()
        assert torch.isfinite(adj_coalesced.values()).all()
        ...
        for count in range(args.runs):
            ...
            for epoch in tqdm(range(0, args.epochs)):
                ...
                if auc_rocs['val'] + F1s['val'] + accs['val'] - args.alpha * (
                    ...
                    # ---- Save best model-induced outputs for echo/polarization analysis ----
                    with torch.no_grad():
                        encoder.eval()
                        classifier.eval()
                        generator.eval()

                        # build a single "effective" x (mean mask) for inference
                        if args.f_mask == 'yes':
                            feature_weights = generator()
                            masks = []
                            for _ in range(args.K):
                                ...
                                masks.append(mask)
                            mask_mean = torch.stack(masks, dim=0).mean(dim=0)
                            x_eff = data.x * mask_mean
                        else:
                            x_eff = data.x

                        h_all = encoder(x_eff, data.edge_index, data.adj_norm_sp)          # (N, hidden)
                        logits_all = classifier(h_all).view(-1)                             # (N,)
                        prob_all = torch.sigmoid(logits_all)                                # (N,)
                        yhat_all = (prob_all > 0.5).long()                                  # (N,)

                        # store on CPU so train.py can use it
                        self.best_prob = prob_all.detach().cpu().numpy()
                        self.best_yhat = yhat_all.detach().cpu().numpy()
                        self.best_emb  = h_all.detach().cpu().numpy()
    ...
    def fit(self,adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx, runs=1, epochs=200, d_epochs=5, g_epochs=5,
            ...
            prop='spmm', dropout=0.5, hidden=16, seed=1, encoder='GCN', K=50, top_k=50, clip_e=1, f_mask='yes',weight_clip='yes',ratio=1, alpha=1):
        ...
        self.args=args
        self.acc, self.f1, self.auc_roc, self.parity, self.equality, self.parity_val, self.equality_val, self.test_acc_sens0, self.test_acc_sens1, self.test_auc_roc_sens0, self.test_auc_roc_sens1, self.test_f1_sens0, self.test_f1_sens1 = self.run(data, args)
    ...
    def predict_val(self):
        parity_val, equality_val = self.parity_val, self.equality_val

        print('parity_val:', np.mean(parity_val))
        print('equality_val:', np.mean(equality_val))

        return parity_val.item(), equality_val.item()