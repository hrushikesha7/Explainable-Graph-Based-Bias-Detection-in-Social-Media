...
def process_pokec_nba(dataset_name='pokec_z', return_tensor_sparse=True, predict_attr_specify=None, feature_normalize=True):
    ...
    # Optional: Mitigate structural bias in the graph
    # Uncomment the following lines to apply graph debiasing
    # from graph_debias import mitigate_structural_bias
    # sens_np = sens.cpu().numpy() if hasattr(sens, "cpu") else sens
    # adj = mitigate_structural_bias(adj, sens_np, method='hybrid', 
    #                                  target_inter_ratio=0.15, max_rewire_ratio=0.1, seed=seed)
...
def process_german_bail_credit(dataset_name,return_tensor_sparse=True, feature_normalize=True):
...
def process_LCC(dataset_name, return_tensor_sparse=True):
    ...
    features=sp.coo_matrix((features['data'], (features['row'], features['col'])),
...
def load_data(dataset_name,  return_tensor_sparse=True, feature_normalize=True):
    ...
    if dataset_name=='facebook':
        ...
        return process_pokec_nba(dataset_name,  return_tensor_sparse, feature_normalize=feature_normalize)
        ...
        return process_german_bail_credit(dataset_name,  return_tensor_sparse, feature_normalize)
