        ...
        for epoch in tqdm(range(args.epochs)):
            ...
            if acc_val > args.acc or epoch==0:
                ...
                if acc_val>best_acc:
                    ...
                    with torch.no_grad():
                        h_all = self.GNN(self.edge_index, self.x)  # (N, hidden) - embeddings
                        logits_all = self.classifier(h_all).view(-1)  # (N,) - logits
                        prob_all = torch.sigmoid(logits_all)  # (N,) - probabilities
                    ...
                    idx_val=idx_val.cpu()
                    F1 = f1_score(self.labels[idx_val].detach().cpu().numpy(), output[idx_val], average='micro')
                    ACC = accuracy_score(self.labels[idx_val].detach().cpu().numpy(), output[idx_val], )
                    if self.labels.max() > 1:
                        AUCROC = 0
                    else:
                        AUCROC = roc_auc_score(self.labels[idx_val].detach().cpu().numpy(), output[idx_val])
                    ACC_sens0, AUCROC_sens0, F1_sens0, ACC_sens1, AUCROC_sens1, F1_sens1 = self.predict_sens_group(
                        output[idx_val], idx_val)
                    SP, EO = self.fair_metric_direct(output[idx_val], self.labels[idx_val].detach().cpu().numpy(),
                                                     self.sens[idx_val].detach().cpu().numpy())

                    self.temp_result_val = ACC, AUCROC, F1, ACC_sens0, AUCROC_sens0, F1_sens0, ACC_sens1, AUCROC_sens1, F1_sens1, SP, EO
