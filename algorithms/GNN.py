...
class GNN(torch.nn.Module):
    def __init__(self, adj, features, labels, idx_train, idx_val, idx_test, sens, sens_idx, num_hidden=16, num_proj_hidden=16, lr=0.001, weight_decay=1e-5, drop_edge_rate_1=0.1, drop_edge_rate_2=0.1, drop_feature_rate_1=0.1, drop_feature_rate_2=0.1, encoder="gcn", sim_coeff=0.5, nclass=1, device="cuda"):
        ...
        self.encoder_name = encoder  # Store encoder name as string for filename
    ...
    def fit(self, epochs=500):
        ...
        for epoch in range(epochs + 1):
            ...
            if (val_loss) < best_loss:
                ...
                best_loss = val_loss
                torch.save(self.state_dict(), f'weights_GNN_{self.encoder_name}.pt')
                
                # Store embeddings, probabilities, and predictions for echo chamber analysis
                with torch.no_grad():
                    self.eval()
                    emb_all = self.forward(self.features.to(self.device), self.edge_index.to(self.device))  # (N, hidden) - embeddings
                    output_all = self.forwarding_predict(emb_all)  # (N, 1) - logits
                    prob_all = torch.sigmoid(output_all.squeeze())  # (N,) - probabilities
                    yhat_all = (prob_all > 0.5).long()  # (N,) - predictions
                    
                    # Store on CPU for echo chamber analysis
                    self.best_prob = prob_all.detach().cpu().numpy()
                    self.best_yhat = yhat_all.detach().cpu().numpy()
                    self.best_emb = emb_all.detach().cpu().numpy()
        
        # If best model variables weren't set, compute them now with final model
        if self.best_prob is None:
            self.eval()
            with torch.no_grad():
                emb_all = self.forward(self.features.to(self.device), self.edge_index.to(self.device))
                output_all = self.forwarding_predict(emb_all)
                prob_all = torch.sigmoid(output_all.squeeze())
                yhat_all = (prob_all > 0.5).long()
    ...
    def predict(self):
        ...
        self.load_state_dict(torch.load(f'weights_GNN_{self.encoder_name}.pt'))
        self.eval()
    ...
    def predict_val(self):
        ...
        output_preds = (output.squeeze() > 0).type_as(self.labels)[self.idx_val].detach().cpu().numpy()

        labels = self.labels.detach().cpu().numpy()
        idx_test = self.idx_val
